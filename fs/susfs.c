...
int susfs_set_i_state_on_external_dir(char __user *user_info, int cmd)
{
	...
	if (cmd == CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH) {
		...
		set_bit(AS_FLAGS_ANDROID_DATA_ROOT_DIR,
			&inode->i_mapping->flags);
		spin_unlock(&inode->i_lock);
		strncpy(android_data_path.pathname, resolved_pathname,
			SUSFS_MAX_LEN_PATHNAME - 1);
		android_data_path.is_inited = true;
		SUSFS_LOGI(
			"Set android data root dir: '%s', i_mapping: '0x%p'\n",
		...
		spin_unlock(&inode->i_lock);
		strncpy(sdcard_path.pathname, resolved_pathname,
			SUSFS_MAX_LEN_PATHNAME - 1);
...
int susfs_add_sus_path(struct st_susfs_sus_path *__user user_info)
{
	...
	if (strstr(resolved_pathname, android_data_path.pathname)) {
		if (!android_data_path.is_inited) {
			err = -EINVAL;
			SUSFS_LOGE(
				"android_data_path is not configured yet, plz do like 'ksu_susfs set_android_data_root_path /sdcard/Android/data' first after your screen is unlocked\n");
			goto out_kfree_tmp_buf;
		}
		list_for_each_entry_safe (cursor, temp,
					  &LH_SUS_PATH_ANDROID_DATA, list) {
			if (unlikely(!strcmp(cursor->info.target_pathname,
					     path.dentry->d_name.name))) {
				spin_lock(&susfs_spin_lock);
				cursor->info.target_ino = info.target_ino;
				strncpy(cursor->info.target_pathname,
					path.dentry->d_name.name,
					SUSFS_MAX_LEN_PATHNAME - 1);
				strncpy(cursor->target_pathname,
					resolved_pathname,
					SUSFS_MAX_LEN_PATHNAME - 1);
				cursor->info.i_uid = info.i_uid;
				cursor->path_len =
					strlen(cursor->info.target_pathname);
				SUSFS_LOGI(
					"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully updated to LH_SUS_PATH_ANDROID_DATA\n",
					cursor->info.target_ino,
		...
		new_list = kmalloc(sizeof(struct st_susfs_sus_path_list),
				   GFP_KERNEL);
		if (!new_list) {
			err = -ENOMEM;
			goto out_kfree_tmp_buf;
		...
		strncpy(new_list->info.target_pathname,
			path.dentry->d_name.name, SUSFS_MAX_LEN_PATHNAME - 1);
		strncpy(new_list->target_pathname, resolved_pathname,
			SUSFS_MAX_LEN_PATHNAME - 1);
		...
		SUSFS_LOGI(
			"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully added to LH_SUS_PATH_ANDROID_DATA\n",
		...
		if (!sdcard_path.is_inited) {
			err = -EINVAL;
			SUSFS_LOGE(
				"sdcard_path is not configured yet, plz do like 'ksu_susfs set_sdcard_root_path /sdcard' first after your screen is unlocked\n");
			goto out_kfree_tmp_buf;
		}
		list_for_each_entry_safe (cursor, temp, &LH_SUS_PATH_SDCARD,
					  list) {
			if (unlikely(!strcmp(cursor->info.target_pathname,
					     path.dentry->d_name.name))) {
				spin_lock(&susfs_spin_lock);
				...
				cursor->path_len =
					strlen(cursor->info.target_pathname);
				SUSFS_LOGI(
					"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully updated to LH_SUS_PATH_SDCARD\n",
					cursor->info.target_ino,
					cursor->target_pathname,
					cursor->info.i_uid);
				spin_unlock(&susfs_spin_lock);
		...
		SUSFS_LOGI(
			"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully added to LH_SUS_PATH_SDCARD\n",
			new_list->info.target_ino, new_list->target_pathname,
			new_list->info.i_uid);
		spin_unlock(&susfs_spin_lock);
		goto out_kfree_tmp_buf;
	...
	SUSFS_LOGI(
		"pathname: '%s', ino: '%lu', is flagged as AS_FLAGS_SUS_PATH\n",
...
int susfs_add_sus_path_loop(struct st_susfs_sus_path *__user user_info)
{
	...
	if (susfs_starts_with(resolved_pathname, "/storage/")) {
		err = -EINVAL;
		SUSFS_LOGE(
			"path starts with /storage and /sdcard cannot be added by add_sus_path_loop\n");
		goto out_kfree_tmp_buf;
	}
	...
	list_for_each_entry_safe (cursor, temp, &LH_SUS_PATH_LOOP, list) {
		if (unlikely(!strcmp(cursor->info.target_pathname,
			...
			spin_lock(&susfs_spin_lock);
			cursor->info.target_ino = info.target_ino;
			strncpy(cursor->info.target_pathname, resolved_pathname,
				SUSFS_MAX_LEN_PATHNAME - 1);
			strncpy(cursor->target_pathname, resolved_pathname,
				SUSFS_MAX_LEN_PATHNAME - 1);
			cursor->info.i_uid = info.i_uid;
			cursor->path_len = strlen(cursor->info.target_pathname);
			SUSFS_LOGI(
				"target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully updated to LH_SUS_PATH_LOOP\n",
				cursor->info.target_ino,
				cursor->target_pathname, cursor->info.i_uid);
			spin_unlock(&susfs_spin_lock);
	...
	new_list->info.target_ino = info.target_ino;
	strncpy(new_list->info.target_pathname, resolved_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	strncpy(new_list->target_pathname, resolved_pathname,
		SUSFS_MAX_LEN_PATHNAME - 1);
	new_list->info.i_uid = info.i_uid;
...
{
	return (base && !IS_ERR(base) && base->d_inode &&
		(base->d_inode->i_mapping->flags &
		 BIT_ANDROID_SDCARD_ROOT_DIR));
}
...
bool susfs_is_sus_android_data_d_name_found(const char *d_name)
{
	...
	list_for_each_entry_safe (cursor, temp, &LH_SUS_PATH_ANDROID_DATA,
		...
		if (!strncmp(d_name, cursor->info.target_pathname,
			     cursor->path_len) &&
		    (d_name[cursor->path_len] == '\0' ||
		     d_name[cursor->path_len] == '/') &&
...
static void susfs_update_sus_mount_inode(char *target_pathname)
{
	...
	if (mnt->mnt_group_id > 0 && // 0 means no peer group
		...
		SUSFS_LOGE(
			"skip setting SUS_MOUNT inode state for path '%s' since its source mount has a legit peer group id\n",
			target_pathname);
		return;
	}
...
int susfs_add_sus_mount(struct st_susfs_sus_mount *__user user_info)
{
	...
	list_for_each_entry_safe (cursor, temp, &LH_SUS_MOUNT, list) {
		if (unlikely(!strcmp(cursor->info.target_pathname,
			...
			susfs_update_sus_mount_inode(
				cursor->info.target_pathname);
			SUSFS_LOGI(
				"target_pathname: '%s', target_dev: '%lu', is successfully updated to LH_SUS_MOUNT\n",
				cursor->info.target_pathname,
				cursor->info.target_dev);
			spin_unlock(&susfs_spin_lock);
			return 0;
		}
	}
	...
	SUSFS_LOGI(
		"target_pathname: '%s', target_dev: '%lu', is successfully added to LH_SUS_MOUNT\n",
...
int susfs_auto_add_sus_bind_mount(const char *pathname,
				  struct path *path_target)
{
	...
	if (!(inode->i_mapping->flags & BIT_SUS_MOUNT)) {
		...
		spin_unlock(&inode->i_lock);
		SUSFS_LOGI(
			"set SUS_MOUNT inode state for source bind mount path '%s'\n",
			pathname);
	}
	return 0;
}
...
void susfs_auto_add_sus_ksu_default_mount(const char __user *to_pathname)
{
	...
	if (strncpy_from_user(pathname, to_pathname,
			      SUSFS_MAX_LEN_PATHNAME - 1) < 0) {
...
set_inode_sus_mount:
	...
	if (!(inode->i_mapping->flags & BIT_SUS_MOUNT)) {
		...
		spin_unlock(&inode->i_lock);
		SUSFS_LOGI(
			"set SUS_MOUNT inode state for default KSU mount path '%s'\n",
			pathname);
	}
out_path_put:
	path_put(&path);
...
int susfs_add_sus_kstat(struct st_susfs_sus_kstat *__user user_info)
{
	...
	if (update_hlist) {
		SUSFS_LOGI(
			...
			new_entry->info.is_statically,
			new_entry->info.target_ino,
			new_entry->info.target_pathname,
			new_entry->info.spoofed_ino,
			new_entry->info.spoofed_dev,
			new_entry->info.spoofed_nlink,
...
int susfs_update_sus_kstat(struct st_susfs_sus_kstat *__user user_info)
{
	...
	hash_for_each_safe (SUS_KSTAT_HLIST, bkt, tmp_node, tmp_entry, node) {
		if (!strcmp(tmp_entry->info.target_pathname,
			    info.target_pathname)) {
			if (susfs_update_sus_kstat_inode(
...
int susfs_add_try_umount(struct st_susfs_try_umount *__user user_info)
{
	...
	list_for_each_entry_safe (cursor, temp, &LH_TRY_UMOUNT_PATH, list) {
		if (unlikely(!strcmp(info.target_pathname,
				     cursor->info.target_pathname))) {
			SUSFS_LOGE(
	...
	spin_unlock(&susfs_spin_lock);
	SUSFS_LOGI(
		"target_pathname: '%s', mnt_mode: %d, is successfully added to LH_TRY_UMOUNT_PATH\n",
...
void susfs_try_umount(uid_t target_uid)
{
	...
	list_for_each_entry_reverse (cursor, &LH_TRY_UMOUNT_PATH, list) {
		if (cursor->info.mnt_mode == TRY_UMOUNT_DEFAULT) {
			...
			try_umount(cursor->info.target_pathname, false,
				   MNT_DETACH, target_uid);
		} else {
			SUSFS_LOGE(
				"failed umounting '%s' for uid: %d, mnt_mode '%d' not supported\n",
				cursor->info.target_pathname, target_uid,
				cursor->info.mnt_mode);
		}
	}
}
...
void susfs_auto_add_try_umount_for_bind_mount(struct path *path)
{
	...
	if (path->dentry->d_inode->i_mapping->flags & BIT_SUS_KSTAT) {
		SUSFS_LOGI(
			"skip adding path to try_umount list as its inode is flagged BIT_SUS_KSTAT already\n");
		return;
	}
#endif
	...
	list_for_each_entry_safe (cursor, temp, &LH_TRY_UMOUNT_PATH, list) {
		...
		if (is_magic_mount_path &&
		    strstr(dpath, cursor->info.target_pathname)) {
			goto out_free_pathname;
		}
#endif
		if (unlikely(!strcmp(dpath, cursor->info.target_pathname))) {
			SUSFS_LOGE(
				"target_pathname: '%s', ino: %lu, is already created in LH_TRY_UMOUNT_PATH\n",
	...
	if (is_magic_mount_path) {
		strncpy(new_list->info.target_pathname,
			dpath + strlen(MAGIC_MOUNT_WORKDIR),
			SUSFS_MAX_LEN_PATHNAME - 1);
		goto out_add_to_list;
	}
#endif
	strncpy(new_list->info.target_pathname, dpath,
		SUSFS_MAX_LEN_PATHNAME - 1);
...
out_add_to_list:
	...
	spin_unlock(&susfs_spin_lock);
	SUSFS_LOGI(
		"target_pathname: '%s', ino: %lu, mnt_mode: %d, is successfully added to LH_TRY_UMOUNT_PATH\n",
		new_list->info.target_pathname, path->dentry->d_inode->i_ino,
		new_list->info.mnt_mode);
out_free_pathname:
	kfree(pathname);
}
...
void susfs_spoof_uname(struct new_utsname *tmp)
{
	if (unlikely(my_uname.release[0] == '\0' ||
		     spin_is_locked(&susfs_uname_spin_lock)))
		return;
...
int susfs_set_cmdline_or_bootconfig(char *__user user_fake_cmdline_or_bootconfig)
{
	...
	if (!fake_cmdline_or_bootconfig) {
		...
		fake_cmdline_or_bootconfig = kmalloc(
	...
	spin_lock(&susfs_spin_lock);
	memset(fake_cmdline_or_bootconfig, 0,
	       SUSFS_FAKE_CMDLINE_OR_BOOTCONFIG_SIZE);
	res = strncpy_from_user(fake_cmdline_or_bootconfig,
				user_fake_cmdline_or_bootconfig,
...
#else
		SUSFS_LOGI(
			"fake_cmdline_or_bootconfig is set, length of string: %u\n",
			strlen(fake_cmdline_or_bootconfig));
#endif
		return 0;
	}
...
susfs_update_open_redirect_inode(struct st_susfs_open_redirect_hlist *new_entry)
{
	...
	err = kern_path(new_entry->target_pathname, LOOKUP_FOLLOW,
			&path_target);
	if (err) {
		SUSFS_LOGE("Failed opening file '%s'\n",
...
int susfs_add_open_redirect(struct st_susfs_open_redirect *__user user_info)
{
	...
	spin_lock(&susfs_spin_lock);
	hash_for_each_safe (OPEN_REDIRECT_HLIST, bkt, tmp_node, tmp_entry,
			    node) {
	...
	if (update_hlist) {
		SUSFS_LOGI(
			"target_ino: '%lu', target_pathname: '%s', redirected_pathname: '%s', is successfully updated to OPEN_REDIRECT_HLIST\n",
...
int susfs_sus_su(struct st_sus_su *__user user_info)
{
	...
	if (info.mode == SUS_SU_WITH_HOOKS) {
		...
		if (last_working_mode != SUS_SU_DISABLED) {
			SUSFS_LOGE(
				"please make sure the current sus_su mode is %d first\n",
				SUS_SU_DISABLED);
			return 2;
		}
